...
module.exports = function(data, wuid, userId, activityData, callback){
    ...
    if (msg == "running" || msg == "before"){
        if  (!!data[msg].cover)  {
            if ( msg == "before") {
                data[msg].url =   "/gaming/scratch/" + msg + "?c=" + _random + "&w=" + wuid  + "&u=" + userId  + "&a=" + activityData["id"];
                ...
                activity.genUserCode(wuid, userId, activityData, function(err, code){
                    if (!err) {
                        data[msg].url =   "/gaming/scratch/" + msg + "?c=" + code + "&w=" + wuid + "&u=" + userId  + "&a=" + activityData["id"];
                        return callback(err, [{
                            title: data[msg].title
                            ,description: data[msg].summary
                            ,picUrl: data[msg].cover
        ...
        return callback(undefined, [{
            ...
            ,url: "/gaming/scratch/" + msg + "?c=" + _random + "&w=" + wuid  + "&u=" + userId + "&a=" +  activityData["id"]
        }]);
